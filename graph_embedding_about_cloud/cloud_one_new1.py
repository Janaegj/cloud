# -*- coding: utf-8 -*-
"""
Created on Wed May  1 17:26:19 2019
...
def build_full_graph(pathtofile, graphtype):
	...
	for line_service in content_service:
			...
			service[i_service].type='service'
...
def get_cloud_graph(ig,node_dict,job,service):
	...
	job_node_idx=choice(job_nodes_idx)#随机选择 
   
    
    
	job_node=get_key(node_dict,job_node_idx) #得到job的ID，便于找寻对应ID的CPU和内存
	job_cpu,job_m=get_cm(NODE,job_node) 
	print(type(job_cpu))
	...
	for i in service_nodes_idx:
            ...
            service_cpu,service_m=get_cm(NODE,service_node)            s_cpu.append (float(service_cpu))
   ...
   #找到对应该服务器的ID号，然后减去运行该job后还能用的内存和CPU
	...
	for i in range(len(NODE)):
	  if NODE[i].ID==service_node[0]:
           print(NODE[i].cpu)
           NODE[i].cpu=float(NODE[i].cpu)-float(job_cpu)
           print(NODE[i].cpu)
           NODE[i].M=float(NODE[i].M)-float(job_m)

	return g,Job,Service,NODE
...
if __name__ == '__main__':
	...
	g, j,s,node=get_cloud_graph(g_undirected,node_dict,job,service)
